...
export default function Home({ onNavigate }: HomeProps) {
  ...
  const quickActions = [
    { icon: FileText, label: 'Apply Service', sublabel: 'सेवा के लिए आवेदन', tab: 'services', color: 'oklch(0.78 0.12 85)' },
    { icon: Package, label: 'My Orders', sublabel: 'मेरे ऑर्डर', tab: 'orders', color: 'oklch(0.6 0.15 200)' },
    { icon: Phone, label: 'Contact Us', sublabel: 'संपर्क करें', tab: 'contact', color: 'oklch(0.6 0.15 145)' },
    { icon: Star, label: 'All Services', sublabel: 'सभी सेवाएं', tab: 'services', color: 'oklch(0.65 0.15 300)' },
  ];
  ...
  return (
    <div className="min-h-full page-enter" style={{ background: 'oklch(0.14 0.04 240)' }}>
      ...
      <div className="relative overflow-hidden px-4 pt-6 pb-8"
        style={{ background: 'linear-gradient(135deg, oklch(0.14 0.04 240) 0%, oklch(0.22 0.06 240) 100%)' }}>
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full opacity-10"
            style={{ background: 'radial-gradient(circle, oklch(0.78 0.12 85), transparent)' }} />
        </div>
          ...
          <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0"
            ...
            <img src="/assets/generated/vijay-logo.dim_512x512.png" alt="Logo"
              ...
              onError={(e) => {
                ...
                if (t.parentElement) t.parentElement.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:oklch(0.22 0.06 240);font-size:1.2rem;font-weight:800;color:oklch(0.78 0.12 85)">V</div>';
              }}
            />
          </div>
          <div>
            <p className="text-xs font-medium" style={{ color: 'oklch(0.72 0.015 240)' }}>
              Welcome back
            </p>
            <h2 className="text-lg font-bold" style={{ color: 'oklch(0.97 0.005 240)' }}>
              {name}
            </h2>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-4"
          ...
          <p className="text-sm font-devanagari font-medium" style={{ color: 'oklch(0.82 0.012 240)' }}>
      ...
      <div className="px-4 py-4">
        <h3 className="text-sm font-semibold mb-3" style={{ color: 'oklch(0.72 0.015 240)' }}>
          QUICK ACCESS
        </h3>
        <div className="grid grid-cols-2 gap-3">
          {quickActions.map((action) => (
            ...
            >
              ...
              <p className="text-xs mt-0.5 font-devanagari" style={{ color: 'oklch(0.62 0.015 240)' }}>
                {action.sublabel}
              </p>
            </button>
          ))}
      ...
      <div className="px-4 pb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {highlights.map((h) => (
            <div key={h.text} className="flex items-center gap-2 p-3 rounded-xl"
              style={{ background: 'oklch(0.18 0.05 240)', border: '1px solid oklch(0.28 0.07 240)' }}>
              <span className="text-lg">{h.emoji}</span>
              <span className="text-xs font-medium" style={{ color: 'oklch(0.82 0.012 240)' }}>{h.text}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pb-6">
        <button
          ...
          style={{ background: 'linear-gradient(135deg, oklch(0.22 0.06 240), oklch(0.28 0.07 240))', border: '1px solid oklch(0.35 0.08 240)' }}
        >
