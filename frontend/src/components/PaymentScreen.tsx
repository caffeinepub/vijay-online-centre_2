...
import { QrCode, Copy, CheckCircle, AlertCircle, Loader2, ArrowLeft } from 'lucide-react';
import { useGetPermQR } from '../hooks/useQueries';
...
export default function PaymentScreen({
  ...
  const { data: permQRBase64 = '', isLoading: qrLoading } = useGetPermQR();
  ...
  useEffect(() => {
    ...
    try {
      ...
      const dots: [number, number][] = [
        ...
        [70, 20], [90, 20], [110, 20],
        [70, 30], [80, 30], [90, 30], [100, 30], [110, 30],
        [70, 40], [90, 40], [110, 40],
        [70, 50], [80, 50], [100, 50], [110, 50],
        [10, 70], [30, 70], [50, 70], [70, 70], [90, 70], [110, 70], [130, 70], [150, 70], [170, 70],
        [20, 80], [40, 80], [60, 80], [80, 80], [100, 80], [120, 80], [140, 80], [160, 80], [180, 80],
        [10, 90], [30, 90], [50, 90], [70, 90], [90, 90], [110, 90], [130, 90], [150, 90], [170, 90],
        [141, 70], [151, 70], [161, 70], [171, 70], [181, 70],
        [141, 80], [161, 80], [181, 80],
        [141, 90], [151, 90], [161, 90], [171, 90], [181, 90],
        [141, 100], [161, 100],
        [141, 110], [151, 110], [171, 110], [181, 110],
        [10, 110], [20, 110], [40, 110], [60, 110], [80, 110], [100, 110],
        [10, 120], [30, 120], [50, 120], [70, 120], [90, 120], [110, 120],
        [10, 130], [20, 130], [40, 130], [60, 130], [80, 130], [100, 130],
        [10, 150], [30, 150], [50, 150], [70, 150], [90, 150], [110, 150], [130, 150], [150, 150], [170, 150],
        [20, 160], [40, 160], [60, 160], [80, 160], [100, 160], [120, 160], [140, 160], [160, 160], [180, 160],
        [10, 170], [30, 170], [50, 170], [70, 170], [90, 170], [110, 170], [130, 170], [150, 170], [170, 170],
        [10, 180], [30, 180], [50, 180], [70, 180], [90, 180], [110, 180], [130, 180], [150, 180], [170, 180],
      ];
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: 'oklch(0.14 0.04 240)' }}>
      {/* Header */}
      <div
      ...
      <div className="px-4 py-6 space-y-5 pb-24">
        ...
        {(serviceName || displayAmount > 0 || customerName) && (
          ...
          >
            ...
            {customerName && (
              <p className="text-sm mt-1" style={{ color: 'oklch(0.72 0.015 240)' }}>for {customerName}</p>
            )}
            {displayAmount > 0 && (
              <p className="text-2xl font-bold mt-2" style={{ color: 'oklch(0.78 0.12 85)' }}>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-sm font-bold" style={{ color: 'oklch(0.97 0.005 240)' }}>
              Scan QR to Pay
            </p>
          </div>
          ...
          <div className="flex flex-col items-center gap-3">
            {qrLoading ? (
              <div
                ...
                style={{ background: 'oklch(0.22 0.06 240)' }}
              >
                <Loader2 className="w-8 h-8 animate-spin" style={{ color: 'oklch(0.78 0.12 85)' }} />
              </div>
            ) : qrImageSrc ? (
              ...
              <div className="p-3 bg-white rounded-xl shadow-inner">
                {qrError ? (
                  <div className="w-48 h-48 flex flex-col items-center justify-center gap-2">
                    ...
                    <p className="text-xs text-gray-500 text-center">QR not available</p>
                  </div>
                ) : (
                  <canvas
                    ref={canvasRef}
            ...
            <p className="text-xs text-center" style={{ color: 'oklch(0.62 0.015 240)' }}>
        ...
        <div
          ...
          style={{ background: 'oklch(0.18 0.05 240)', border: '1px solid oklch(0.28 0.07 240)' }}
        >
          <p className="text-xs font-medium mb-2" style={{ color: 'oklch(0.62 0.015 240)' }}>UPI ID</p>
          <div
            className="flex items-center gap-3 rounded-xl px-4 py-3"
            style={{ background: 'oklch(0.22 0.06 240)' }}
          >
            <span className="flex-1 font-mono text-sm" style={{ color: 'oklch(0.97 0.005 240)' }}>{upiId}</span>
            <button
            ...
            >
              {copied ? (
                ...
                <Copy className="w-4 h-4" style={{ color: 'oklch(0.78 0.12 85)' }} />
              )}
            </button>
          </div>
          {copied && (
            <p className="text-xs text-green-400 mt-2 text-center">UPI ID copied!</p>
          )}
        </div>
        ...
        >
          ...
          <ol className="space-y-2">
            {[
              ...
              displayAmount > 0 ? `3. Enter amount: ₹${displayAmount.toLocaleString('en-IN')}` : '3. Enter the payment amount',
              '4. Complete the payment',
              '5. Take a screenshot of the confirmation',
              '6. Share it on WhatsApp for faster processing',
            ].map((step, i) => (
              <p key={i} className="text-xs" style={{ color: 'oklch(0.72 0.015 240)' }}>{step}</p>
            ))}
          </ol>
        </div>
